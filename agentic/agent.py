...
Architecture upgrade from create_react_agent → explicit node graph:

    START → [PLANNER] → [EXECUTOR] ⇄ [TOOLS] → [SYNTHESIZER] → END

Nodes:
  PLANNER    — cheap mini-model generates a step-by-step retrieval plan before
               any tools are called. Plan is stored in state and shown to user.
  EXECUTOR   — gpt-4o with bound tools decides the next action (tool call or done).
               Receives the plan as a system-level instruction on its first call.
  TOOLS      — ToolNode executes whatever tool the executor selected.
  SYNTHESIZER — structured final answer generation using Pydantic guardrails.
...
class AgentGraphState(TypedDict):
    ...
    iteration: int     # Counts EXECUTOR→TOOLS cycles
...
Available tools:
  • search_knowledge_base — semantic search across all READMEs
  • get_full_document     — fetch the complete README for a specific repo
  • list_repos            — list all repos in the knowledge base
  • get_github_stats      — live GitHub data (stars, issues, activity)
...
def _build_graph(llm_executor: ChatOpenAI, llm_synthesizer: ChatOpenAI):
    ...
    def synthesizer(state: AgentGraphState) -> dict:
        ...
        try:
            ...
            # Guardrails failed — final executor AIMessage already has the answer
            return {}
...
def run_agentic_pipeline(
    ...
    Args:
        ...
        max_iterations: Max EXECUTOR→TOOLS loops before the graph is force-stopped.
